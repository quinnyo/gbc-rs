...
function M.setup_autocmd()
    ...
    local events = "BufEnter,BufWinEnter,TabEnter"
    ...
    vim.api.nvim_command('autocmd ' .. events ..
                             ' *.gbc :lua require"gbc-tools.inlay_hints".set_inlay_hints()')
...
local namespace = vim.api.nvim_create_namespace("gbc-analyzer/inlayHints")
local namespace_opt = vim.api.nvim_create_namespace("gbc-analyzer/inlayHintsOptimizer")
...
local function handler(err, result, ctx)
    ...
    for key, value in pairs(ret) do
        ...
        if (current_line) then
            local virt_text = ""
            local optimizer_virt_text = ""
            local current_line_len = string.len(current_line)

            local param_hints = {}
            local other_hints = {}
            local optimizer_hints = {}
            ...
            for _, value_inner in ipairs(value) do
                ...
                elseif value_inner.kind == "OptimizerHint" then
                    table.insert(optimizer_hints, value_inner.label)
                else
            ...
            -- show optimizer hints with commas and a thicc arrow
            if not vim.tbl_isempty(optimizer_hints) then
                optimizer_virt_text = optimizer_virt_text .. opts.optimizer_hints_prefix
                for i, value_inner_inner in ipairs(optimizer_hints) do
                    optimizer_virt_text = optimizer_virt_text .. value_inner_inner
                    if i ~= #optimizer_hints then
                        optimizer_virt_text = optimizer_virt_text .. ", "
                    end
                end
            end
            ...
            if config.options.tools.inlay_hints.max_len_align then
                optimizer_virt_text = string.rep(" ", max_len - current_line_len +
                                           config.options.tools.inlay_hints
                                               .max_len_align_padding) ..
                                optimizer_virt_text
            end
            ...
            if optimizer_virt_text ~= "" then
                vim.api.nvim_buf_set_extmark(bufnr, namespace_opt, line, 0, {
                    virt_text_pos = config.options.tools.inlay_hints.right_align and
                        "right_align" or "eol",
                    virt_text = {
                        {optimizer_virt_text, config.options.tools.inlay_hints.highlight_optimizer}
                    },
                    hl_mode = "combine"
                });
            end
...
function M.disable_inlay_hints()
    ...
    vim.api.nvim_buf_clear_namespace(0, namespace_opt, 0, -1)
end

-- Sends the request to gbc-analyzer to get the inlay hints and handle them
function M.set_inlay_hints()
    utils.command("view/inlay_hints", handler)
end
